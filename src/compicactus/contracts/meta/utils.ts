...
export function getExecuteMetaTransactionData(
  ...
  return [
    ...
    padEnd(64 * signaturePadding, '0', method)
  ].join('')
}

function padEnd(count:number, char:string, str:string) {
...
function padStart(count:number, char:string, str:string) {
    for (let n=0; n<count; n++) {
        if (str.length>=count) break
        str = `${char}${str}`
    }
...
function to32Bytes(value: number | string): string {
  ...
  return value
    ...
    return padStart(64, '0', value.toString().replace('0x', ''))
}
...
export function getSignatureParameters (signature: string) {
    ...
    const v_s = "0x".concat(signature.slice(130, 132));
    //v = web3.utils.hexToNumber(v);
    let v = parseInt(v_s, 16);
    if (!([27, 28].indexOf(v) > -1)) v += 27;
    return {
        r: r,
        s: s,
        v: v
